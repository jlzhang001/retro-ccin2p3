...
if DISPLAY==1:  # Comparison to initial study
  files = ["share/HS1freespace.primaries.5ants.3s.v2.50200"]
#  ,"HS1ground.primaries.5ants.3s.50200",,"HS1ground.primaries.5ants.3s.noAtt.50200",]
  col = ["k","k","g"]
  lin = ["-","--","-"]
...
for i in range(len(files)):
    ...
    y = p/ primary_flux(x)
    lim = 2.44/(numpy.trapz(y/x,numpy.log(x))*dur)
    lim200k_allflavor = lim/20*3
    print lim,lim200k_allflavor

    if i==0:
      # Differential limit
      dlim = 2.44*x/(numpy.log(10)*y*dur)
      dlim200k = dlim/20*3  # All flavor
      ...
      plt.loglog(eGRANDini,lGRANDini,linestyle='-',color="orange",lw=4,label="GRAND ini (agr)")
      plt.xlabel(r"energy, E$_\nu$ (GeV)")
      plt.ylabel(r"Neutrino sensitivity E$^2\Phi(E)$ (GeV.s$^-1$.cm$^-2$.sr$^-1$)")
...
rca = yv[0,:]/exppreci  # Conservative prel to Conservative now
plt.figure(3)
plt.semilogx(xi,rpa,'orange',lw=3,label='Present/ini (Agr)')
plt.semilogx(xi,rca,'magenta',lw=3,label='Present/ini (Cons)')
plt.grid(True)
plt.legend(loc='best')
